...
public class RabbitMQConfig {
    ...
    @Bean
    public Queue cardOrderSubmissionQueue() {
        return new Queue(RabbitMQConstants.QUEUE_CARD_ORDER_SUBMISSION);
    }

    @Bean
    public Binding cardOrderSubmissionBinding(Queue cardOrderSubmissionResultQueue,
                                                    DirectExchange cmsExchange) {
        return BindingBuilder.bind(cardOrderSubmissionResultQueue)
                .to(cmsExchange)
                .with(RabbitMQConstants.ROUTING_KEY_CARD_ORDER_SUBMISSION);
    }
